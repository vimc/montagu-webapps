...
describe('Singleton variable cache tests', () => {
    ...
    test('sets data to a prop with path and gets data by same path', () => {
    ...
    test('sets data to a prop by path and gets data of parent prop', () => {
        const cache = new SingletonVariableCache();
        cache.set(key, "testData");
    ...
    test('sets data to a prop by path and clears by same path', () => {
        const cache = new SingletonVariableCache();
        cache.set(key, "testData");
        expect(cache.get(key)).to.equal("testData");
        cache.clear(key);
    ...
    test('sets data to a prop by path and clears all', () => {
        const cache = new SingletonVariableCache();
        cache.set(key, "testData");
        expect(cache.get(key)).to.equal("testData");
        cache.clearAll();
